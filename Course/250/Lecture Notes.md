...
1. **光是电磁波**  
    虽然我们常画“光线”（一条线），但更真实的描述是：光是波，像水面上的波一样。
    
2. **波前（wave-front）是什么？**  
    想像水面波动，一圈一圈的同心圆。每一条圆弧，就是**波峰同时到达的位置**——这就叫 **波前**。  
    对于远处来的平面波，波前可以画成一条条平行的直线（PPT 里红色的一条条斜线）。
    
3. **光线（ray）是什么？**  
    为了简化，我们不总是画一大片波，而是画一条指示传播方向的线，这条线**总是垂直于波前**。  
    所以：
...
4. **入射、反射、折射角**
    ...
    - 法线（normal）是垂直于界面的那条竖线。
        
    - 入射角：入射光线和法线之间的夹角，标记为 `θ_i`。
        
    - 反射角：反射光线与法线的夹角，`θ_r`。
        
    - 折射角：进入另一种介质后光线和法线的夹角，`θ_t`。
        
5. **折射率 n 与光速**  
    真空中的光速 $c_0 \approx 3\times 10^8\ \text{m/s}$。在介质中速度变慢：  
    $$v = \frac{c_0}{n}$$  
    折射率 $n$ 越大，光在里面跑得越慢（例如：空气 $n\approx1$，水约 1.33，玻璃约 1.5）。
...
- 左边：入射波（红色斜条）打到界面（绿色线），光线从左往右斜着下来，角度是 $θ_i$。
    
- 右边：反射波，光线反弹回去，角度是 $θ_r$。
    
- 蓝色竖线是画来辅助几何的，不是真实的物理界面。
    
- 关键思想：**波前在边界上是连续的**。
    
    > 就像水波拍到直岸边，岸边某一条线上的水面“起伏形态”必须是一致的，否则水面会断开。
    

这句话在 PPT 上写成：

> The electric field wave-fronts are continuous at a boundary.

### 1.2 用“走一小段时间 Δt”来想

想像某一瞬间，波前经过点 A（界面上的一点）。再过一小段时间 $Δt$：

- 入射波在介质中继续往下走，传播距离 = 入射速度 $v_i$ × 时间 $Δt$。  
    这段距离在图中就是 $BD$：  
    $$BD = v_i Δt$$
    
- 同时，反射波也从 A 向另一边传播，过了同样的时间 $Δt$，波前移动到点 E，所以  
    $$AE = v_r Δt$$
    

因为反射还在**同一种介质**里，速度 $v_i = v_r = c_0/n_i$，所以：  
$$BD = v_iΔt = \frac{c_0}{n_i}Δt$$  
$$AE = v_rΔt = \frac{c_0}{n_i}Δt$$  
于是：  
$$BD = AE$$
...
> But:  
> $BD = v_i Δt = (c_0/n_i)Δt$  
> &  
> $AE = v_t Δt = (c_0/n_i)Δt$

（这里 $v_t$ 实际上就是反射波的速度，仍然是 $c_0/n_i$）

### 1.3 三角形几何关系

看左图：

- 三角形 ABD 是一个直角三角形，直角在 B。
    
- $θ_i$ 是入射角（光线和法线夹角），在 D 点那里画出来。
    

在这个直角三角形里，沿界面的那一段 $AD$ 与斜边 $BD$ 的关系是：  
$$AD = \frac{BD}{\sin θ_i}$$

类似地，在右图：

- 三角形 ADE 也是直角三角形，直角在 E。
...
把两式合并：  
$$\frac{BD}{\sin θ_i} = \frac{AE}{\sin θ_r} \quad\Rightarrow\quad  
AD = AD$$  
（其实就是“同一段 AD 用两种方式表达”。）
...
上面几何给出：  
$$\frac{BD}{\sin θ_i} = \frac{AE}{\sin θ_r}$$

前面速度给出：$BD = AE$。

带进去：  
$$\frac{BD}{\sin θ_i} = \frac{BD}{\sin θ_r}$$

把 BD 消掉：  
$$\frac{1}{\sin θ_i} = \frac{1}{\sin θ_r}  
\quad\Rightarrow\quad  
\sin θ_i = \sin θ_r$$

两角都在 $0^\circ \sim 90^\circ$，所以：  
$$θ_i = θ_r$$
...
现在看第二张图：**Refraction and Snell’s Law**。

这里和刚才唯一不同的是：

- 入射介质折射率：$n_i$，速度 $v_i = c_0/n_i$
    
- 透射介质折射率：$n_t$，速度 $v_t = c_0/n_t$
    
- 两边不是同一种介质，所以 $v_i ≠ v_t$。
    

### 2.1 波前还是要连续

同样，我们在某一时刻看：波前刚好通过 A 点。

再过一小段时间 $Δt$：

- 入射侧的波前走到 B，距离 $BD = v_iΔt$。
    
- 透射（折射）侧的波前走到 E，距离 $AE = v_tΔt$。
    

图上还是用那条沿界面的 $AD$ 来连接两边的几何关系。
...
两式相等：  
$$\frac{BD}{\sin θ_i} = \frac{AE}{\sin θ_t}$$

PPT 对应：

> So: $BD/\sin(θ_i) = AE/\sin(θ_t)$
...
- 透射侧：  
    $$AE = v_tΔt = \frac{c_0}{n_t}Δt$$
    

带进几何关系：  
$$\frac{(c_0/n_i)Δt}{\sin θ_i}  
= \frac{(c_0/n_t)Δt}{\sin θ_t}$$

$Δt$ 和 $c_0$ 都是公共因子，可以约掉：
...
这就是**斯涅尔定律（Snell’s law）**，PPT 最后一行框起来的那句。

---

## 3. 从“水波直觉”再理解一次

如果你还觉得抽象，可以用水波的比喻再想一遍。

### 3.1 反射

- 水波斜着打到一堵“硬墙”（比如水池边）。
    
- 墙上的某一条直线处，波峰必须连成一条连续的线，看上去不能出现断点。
    
- 同一侧水里的波速不变，只是方向反了。
    
- 要想让每一个波峰都连得上，几何上刚好要求：入射角 = 反射角。  
    否则，过一段时间之后，波峰在边界上的位置就对不上，边界就“撕裂了”。
    

### 3.2 折射

- 想像水波从深水区（波速大）进入浅水区（波速小），靠近浅水区的一部分波前先减速。
    
- 就好像一辆车从公路开到泥地，一侧轮胎先进到泥地，先变慢，于是整辆车会转向——这就是折射。
    
- 想要波前在边界上仍然连续，就会自动给出一个几何关系：  
    $n_i \sin θ_i = n_t \sin θ_t$。  
    深水（速度大，折射率小）对应折射角大；浅水（速度小，折射率大）对应折射角小。
...
这些就是学习后面镜头、光纤、干涉、衍射等一切内容的地基。

---

如果你愿意，下一步我可以带你做两件事（你可以直接选一个让我讲）：

1. **用费马原理（走时间最短的路径）重新推导反射和折射定律**，再从“最短时间”角度理解一次。
    
2. **做几个具体小例题**：比如从空气进入水中、玻璃中，计算折射角、全反射条件等等，让公式真正“落地”。