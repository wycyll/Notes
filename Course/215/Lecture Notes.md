...
- 最大功率条件：负载阻抗需等于戴维南阻抗的复数共轭，即 $Z_L = Z_{\text{TH}}^* = R_{\text{TH}} - jX_{\text{TH}}$；
    - 物理意义：负载的负电抗（如电容）抵消戴维南电路的正电抗（如电感），使总阻抗呈纯电阻性，避免能量在电抗间往返交换，全部用于电阻消耗
- 最大平均功率公式：$P_{\text{max}} = \frac{V_{\text{TH}}^2}{8R_{\text{TH}}}$, $V_{TH}$ 是峰值，注意有 1/2
### 纯电阻负载
若负载无电抗（$X_L = 0$），则最大功率条件简化为 $R_L = \sqrt{R_{\text{TH}}^2 + X_{\text{TH}}^2}$（负载电阻等于戴维南阻抗的模），此时功率小于 “复数共轭匹配” 的最大值（工程中若无法调整电抗，可采用此近似）
